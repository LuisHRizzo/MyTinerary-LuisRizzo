...
const SignUp = ({ screenFn }) => {
    ...
    const getCountries = async () => {
        try {
            const res = await axios.get('http://localhost:3000/api/countries/');
    ...
    const loginGoogle = (credential) => {
        ...
        formData.password = "Aa_123$" + res.sub;
    ...
    return (
        <form onSubmit={submitFn} className='form-group login-box d-flex justify-content-evenly align-items-center flex-column h-100'>
            <h2 className='text-center form-group '>Sign Up</h2>

            <label className=' fs-5'>
                <i className='bi bi-person-fill-add login-icon'></i>

                <input type='text' className='form-group login-input' ref={firstName} placeholder='First Name' />
            </label>

            <label className='form-group fs-5'>
                <i className='bi bi-person-add login-icon'></i>

                <input type='text' className='form-group login-input' ref={lastName} placeholder='Last Name' />
            </label>
            ...
            <label className='form-group fs-5 d-flex'>
